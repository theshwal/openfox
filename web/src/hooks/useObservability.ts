...
import { computeObservability, computeObservabilityFromSessionStats } from '@shared/observability.js'
import type { Message, ObservabilitySnapshot, ObservabilityStats, SessionStats } from '@shared/types.js'
...
export function useObservability(
  messages: Message[],
  snapshot: ObservabilitySnapshot | null | undefined,
  fullStats?: SessionStats | null,
): ObservabilityStats | null {
  return useMemo(() => {
    // When fullStats is available (StatsModal lazy-load), prefer it: it carries
    // the server-computed per-call cache attribution directly.
    if (fullStats) {
      return computeObservabilityFromSessionStats(fullStats)
    }
    const events = buildEventsFromSnapshot(messages, snapshot)
    return computeObservability(messages, events)
  }, [messages, snapshot, fullStats])
}
