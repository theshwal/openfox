      ...
      try {
        ...
        try {
          for await (const chunk of stream) {
            ...
            if (chunk.usage) {
              // Extract every chunk's usage through the provider-agnostic normalizer
              // so cache fields (cachedPromptTokens / cacheWriteTokens / cacheSource)
              // survive the streaming path. The last non-empty chunk wins.
